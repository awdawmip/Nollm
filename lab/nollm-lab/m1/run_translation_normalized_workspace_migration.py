...
SOURCE_PROFILE_REGISTRY_VERSION = "nollm_geometry_profiles_v4"
SOURCE_KERNEL_REGISTRY_VERSION = "nollm_geometry_kernels_v4"
...
def migrate(source: Path, target: Path) -> dict[str, object]:
    ...
    old_registry = core["geometry_registry"]
    if type(old_registry) is not dict or old_registry.get("profile_registry_version") != SOURCE_PROFILE_REGISTRY_VERSION or old_registry.get("kernel_registry_version") != SOURCE_KERNEL_REGISTRY_VERSION:
        raise ValueError("source is not the accepted translation-covariant geometry registry")
    ...
    try:
        ...
        receipt = {
            "schema_version": "nollm_translation_normalized_workspace_migration_v1",
            "source_workspace": str(source),
            "target_workspace": str(target),
            "mapping": "physical_addresses_unchanged;geometry_registry_v4_to_v5",
