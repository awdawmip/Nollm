...
def migrate(source: Path, target: Path) -> dict[str, object]:
    ...
    if type(old_registry) is not dict or old_registry.get("profile_registry_version") != "nollm_geometry_profiles_v3" or old_registry.get("kernel_registry_version") != "nollm_geometry_kernels_v3":
        raise ValueError("source is not the accepted V3.7 geometry registry")
    ...
    try:
        ...
        receipt = {
            "schema_version": "nollm_translation_covariant_workspace_migration_v1",
            "source_workspace": str(source),
            "target_workspace": str(target),
            "mapping": "physical_addresses_unchanged;geometry_registry_v3_to_v4",
