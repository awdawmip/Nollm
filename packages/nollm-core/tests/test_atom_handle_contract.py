...
def test_minimal_atom_and_addressed_handle_contracts() -> None:
    ...
    address = GeometryAddress("eisenstein_exact_v1", "chart", 2, -17, 9)
    handle = AtomHandle(address, "local-a")
    assert handle.geometry_address.stable_key() == ("eisenstein_exact_v1", "chart", 2, -17, 9, "")
...
def test_same_atom_id_is_valid_in_independent_core_instances(tmp_path) -> None:
    ...
    cell = GeometryAddress("eisenstein_exact_v1", "chart", 0, 0, 0)
