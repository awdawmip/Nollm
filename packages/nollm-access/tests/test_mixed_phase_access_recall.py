...
from nollm_core import CoreRuntime, GeometryAddress, RecallBudget
...
def test_mixed_phase_access_entry_order(tmp_path) -> None:
    ...
    for i,cell in enumerate(cells): access.capture(MemoryStatement(str(i),str(i))); access.apply(AccessDecision(str(i),str(i),'new',target_cell=cell,reason_text='x'))
