...
PROGRESSIVE_ATLAS_SCHEMA_VERSION = "nollm_access_progressive_atlas_page_v2"
...
class ProgressiveAtlasPolicy:
    ...
    def __post_init__(self) -> None:
        if (
            type(self.max_regions_per_page) is not int
            or not 1 <= self.max_regions_per_page <= 32
        ):
            raise ValueError("max_regions_per_page must be in [1,32]")
        if (
            type(self.max_prompt_bytes) is not int
            or not 1024 <= self.max_prompt_bytes <= 65536
        ):
...
class ProgressiveAtlasRegion:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            type(self.geometry_identity) is not dict
            or type(self.aggregation_order) is not int
            or not 0 <= self.aggregation_order <= 8
        ):
        ...
        if any(
            ...
            for value in (
                ...
                self.native_atom_count,
                self.child_region_count,
            )
        ):
            raise ValueError("region counts must be non-negative")
        if (
            type(self.support_entries) is not tuple
            or not 1 <= len(self.support_entries) <= 4
        ):
        ...
        for entry in self.support_entries:
            ...
            if (
                type(entry["occupancy_count"]) is not int
                or entry["occupancy_count"] < 0
            ):
        ...
        if (
            type(self.representative_statements) is not tuple
            or len(self.representative_statements) > 2
        ):
            raise ValueError("region representatives must be bounded")
        for preview in self.representative_statements:
            if type(preview) is not dict or set(preview) != {
                "statement_id",
                "content_utf8",
                "truncated",
            }:
                raise ValueError("region representative preview is not canonical")
            if (
                type(preview["statement_id"]) is not str
                or not preview["statement_id"]
                or type(preview["content_utf8"]) is not str
                or type(preview["truncated"]) is not bool
            ):
                raise TypeError("region representative preview fields are invalid")
            if len(preview["content_utf8"]) > 64:
                raise ValueError(
                    "region representative exceeds the uniform codepoint budget"
                )
    ...
    def to_mapping(self) -> dict[str, object]:
        return {
            ...
            "leaf": self.leaf,
            "local_detail_available": self.native_atom_count
            > len(self.representative_statements),
        }
    ...
    def from_mapping(cls, value: object) -> "ProgressiveAtlasRegion":
        ...
        if (
            type(value["support_entries"]) is not list
            or type(value["representative_statements"]) is not list
        ):
            raise ValueError("invalid Progressive Atlas region arrays")
        result = cls(
            value["region_id"],
            value["geometry_identity"],
            value["aggregation_order"],
            value["source_cell_count"],
            value["native_atom_count"],
            value["child_region_count"],
            tuple(value["support_entries"]),
            tuple(value["representative_statements"]),
            value["truncated"],
            value["support_overflow"],
        )
        if value["leaf"] != result.leaf or value["local_detail_available"] != (
            result.native_atom_count > len(result.representative_statements)
        ):
            raise ValueError(
                "Progressive Atlas region derived fields are not canonical"
            )
        return result
...
class ProgressiveAtlasPage:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or type(self.scope) is not PhysicalFieldScope
        ):
            raise TypeError("page request and scope are required")
        if any(
            type(value) is not str or len(value) != 64
            for value in (
                self.core_state_sha256,
                self.atlas_fingerprint,
                self.page_fingerprint,
            )
        ):
            raise ValueError("page fingerprints must be SHA-256 text")
        if self.parent_region_id is not None and (
            type(self.parent_region_id) is not str or not self.parent_region_id
        ):
        ...
        if self.aggregation_order is not None and (
            type(self.aggregation_order) is not int
            or not 0 <= self.aggregation_order <= self.policy.max_order
        ):
            raise ValueError("page aggregation order is invalid")
        if (
            type(self.regions) is not tuple
            or len(self.regions) > self.policy.max_regions_per_page
        ):
        ...
        for entry in entries:
            ...
            if previous != cell:
                raise ValueError(
                    "one selectable entry identity cannot name multiple Cells"
                )
        counts = (
            self.occupied_field_cell_count,
            self.covered_source_cell_count,
            self.uncovered_source_cell_count,
            self.serialized_utf8_bytes,
    ...
    def from_mapping(cls, value: object) -> "ProgressiveAtlasPage":
        ...
        if (
            ...
            or set(value) != keys
            or value.get("schema_version") != PROGRESSIVE_ATLAS_SCHEMA_VERSION
        ):
        ...
        if (
            type(value["regions"]) is not list
        ...
        regions = tuple(
            ProgressiveAtlasRegion.from_mapping(item) for item in value["regions"]
        )
        if certificate["region_count"] != len(regions):
            raise ValueError("Progressive Atlas region count is not canonical")
        return cls(
            value["request_id"],
            PhysicalFieldScope.from_mapping(value["field_scope"]),
            value["core_state_sha256"],
            value["atlas_fingerprint"],
            value["page_fingerprint"],
            value["parent_region_id"],
            value["depth"],
            value["aggregation_order"],
            regions,
            certificate["occupied_field_cell_count"],
            certificate["covered_source_cell_count"],
            certificate["uncovered_source_cell_count"],
            certificate["serialized_utf8_bytes"],
            certificate["estimated_token_units"],
