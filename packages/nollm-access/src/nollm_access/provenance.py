...
LEGACY_PROVENANCE_SCHEMA_VERSION = "nollm_access_statement_provenance_v1"
PROVENANCE_SCHEMA_VERSION = "nollm_access_statement_provenance_v2"
MIGRATION_PRECISIONS = {"exact", "coarse", "legacy"}
ORIGIN_KINDS = {"assistant", "model_inference", "recalled_memory", "tool", "user"}
...
class ResolvedReferenceProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        for name, values in (
            ("basis_evidence_ref_ids", self.basis_evidence_ref_ids),
            ("timestamp_basis_capture_ids", self.timestamp_basis_capture_ids),
        ):
    ...
    def from_mapping(cls, value: object) -> "ResolvedReferenceProvenance":
        ...
        if (
            ...
            or set(value) != keys
            or type(value["basis_evidence_ref_ids"]) is not list
            or type(value["timestamp_basis_capture_ids"]) is not list
        ):
            raise ValueError(
                "ResolvedReferenceProvenance mapping must have exact fields"
            )
        return cls(
            value["reference_id"],
            value["kind"],
            value["normalized_value"],
            tuple(value["basis_evidence_ref_ids"]),
            tuple(value["timestamp_basis_capture_ids"]),
        )
...
class StatementProvenance:
    ...
    def __post_init__(self) -> None:
        ...
        for name, values in (
            ...
            ("context_capture_ids", self.context_capture_ids),
        ):
            if type(values) is not tuple or any(
                type(item) is not str or not item for item in values
            ):
        ...
        if type(self.evidence_spans) is not tuple or any(
            type(item) is not ExactEvidenceSpan for item in self.evidence_spans
        ):
            raise TypeError("evidence_spans must be ExactEvidenceSpan tuple")
        if type(self.resolved_references) is not tuple or any(
            type(item) is not ResolvedReferenceProvenance
        ...
        if any(
            not set(item.basis_evidence_ref_ids) <= ref_ids
            for item in self.resolved_references
        ):
            raise ValueError(
                "resolved-reference basis must cross-link exact Evidence refs"
            )
        available_capture_ids = set(self.source_capture_ids) | set(
            self.context_capture_ids
        )
        if any(
            item.capture_id not in available_capture_ids for item in self.evidence_spans
        ):
            raise ValueError("Evidence span names an undeclared Capture")
        if any(
            not set(item.timestamp_basis_capture_ids) <= available_capture_ids
            for item in self.resolved_references
        ):
        ...
        if self.revision_predecessor_statement_id is not None:
            _text(
                "revision_predecessor_statement_id",
                self.revision_predecessor_statement_id,
            )
        ...
        if self.schema_version not in {
            ...
            raise ValueError("invalid Statement provenance schema version")
        for name, values in (
            ("origin_kinds", self.origin_kinds),
            ("derived_from_statement_ids", self.derived_from_statement_ids),
        ):
            if type(values) is not tuple or any(
                type(item) is not str or not item for item in values
            ):
            ...
            if tuple(sorted(set(values))) != values:
                raise ValueError(f"{name} must be sorted and unique")
        if any(item not in ORIGIN_KINDS for item in self.origin_kinds):
            raise ValueError("invalid origin kind")
        if self.evaluation_id is not None:
            _text("evaluation_id", self.evaluation_id)
        if type(self.continuation_pass) is not int or self.continuation_pass < 0:
            raise TypeError("continuation_pass must be a non-negative integer")
        if self.schema_version == LEGACY_PROVENANCE_SCHEMA_VERSION and (
            self.origin_kinds
            or self.derived_from_statement_ids
            or self.evaluation_id is not None
            or self.continuation_pass != 0
        ):
            raise ValueError("legacy provenance cannot contain v2 fields")
    ...
    def to_mapping(self) -> dict[str, object]:
        value = {
            ...
            "resolved_references": [
                item.to_mapping() for item in self.resolved_references
            ],
            ...
            value.update(
                {
                    ...
                    "continuation_pass": self.continuation_pass,
                }
            )
        return value
    ...
    def from_mapping(cls, value: object) -> "StatementProvenance":
        ...
        if (
            type(value) is not dict
            or (
                value.get("schema_version") == LEGACY_PROVENANCE_SCHEMA_VERSION
            ...
            not in {LEGACY_PROVENANCE_SCHEMA_VERSION, PROVENANCE_SCHEMA_VERSION}
        ):
        ...
        return cls(
            ...
            tuple(value["source_capture_ids"]),
            tuple(value["context_capture_ids"]),
            tuple(
                ExactEvidenceSpan.from_mapping(item) for item in value["evidence_spans"]
            ),
            tuple(
                ResolvedReferenceProvenance.from_mapping(item)
                for item in value["resolved_references"]
            ),
            value["writer_schema_version"],
            value["writer_prompt_version"],
            value["created_at_epoch_ms"],
            value["revision_predecessor_statement_id"],
            value["migration_precision"],
            tuple(value.get("origin_kinds", ())),
            tuple(value.get("derived_from_statement_ids", ())),
            value.get("evaluation_id"),
            value.get("continuation_pass", 0),
...
class FileStatementProvenanceStore:
            ...
            if path.is_file():
                if path.read_bytes() != payload:
                    raise FileExistsError(
                        "Statement provenance already exists with different content"
                    )
                return
    ...
    def get(self, statement_id: str) -> StatementProvenance:
        ...
        if (
            provenance.statement_id != statement_id
            or provenance.canonical_bytes() != payload
        ):
    ...
    def discard_new(self, provenance: StatementProvenance) -> bool:
        ...
        with self._lock:
            ...
            if path.read_bytes() != provenance.canonical_bytes():
                raise FileExistsError(
                    "Statement provenance exists with different content"
                )
            path.unlink()
            return True
