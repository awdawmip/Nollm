...
class EvidenceQuoteRef:
    ...
    def from_mapping(cls, value: object) -> "EvidenceQuoteRef":
        required = {"evidence_ref_id", "capture_id", "role", "quote_utf8"}
        ...
        return cls(
            value["evidence_ref_id"], value["capture_id"], value["role"], value["quote_utf8"],
...
class ExactEvidenceSpan:
    ...
    def from_mapping(cls, value: object) -> "ExactEvidenceSpan":
        keys = {"evidence_ref_id", "capture_id", "role", "start", "end", "quote_utf8", "quote_sha256", "capture_sha256"}
        if type(value) is not dict or set(value) != keys:
            raise ValueError("ExactEvidenceSpan mapping must have exact fields")
        return cls(*(value[key] for key in ("evidence_ref_id", "capture_id", "role", "start", "end", "quote_utf8", "quote_sha256", "capture_sha256")))
...
def resolve_evidence_quote_refs(refs: object, captures: object) -> tuple[ExactEvidenceSpan, ...]:
    ...
    for capture in captures:
        if type(capture) is not dict or set(capture) != CAPTURE_KEYS:
            raise ValueError("Capture mapping must have exact fields")
        capture_id = _required_text("capture_id", capture["capture_id"])
...
def _resolve_quote(ref: EvidenceQuoteRef, captures: dict[str, dict[str, object]]) -> ExactEvidenceSpan:
    ...
    content = capture[f"{ref.role}_utf8"]
    starts: list[int] = []
    cursor = 0
    while True:
    ...
    return ExactEvidenceSpan(
        ...
        sha256(_canonical(capture)).hexdigest(),
    )
