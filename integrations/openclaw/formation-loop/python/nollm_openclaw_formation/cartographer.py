...
PREVIOUS_PROPOSITION_WRITER_SCHEMA_VERSION = (
...
PROPOSITION_WRITER_PROMPT_VERSION = (
    "proposition-writer-v4-content-neutral-evidence-continuation"
)
...
def read_memory_evaluation_fingerprint(memory_workspace: object) -> dict[str, object]:
...
def build_proposition_writer_prompt(
    ...
    if type(max_statements) is not int or not 1 <= max_statements <= 8:
        raise FormationAdapterError(
            "invalid_writer_budget", "max_statements must be in [1,8]"
        )
    ...
    if (
        type(current_memory_fingerprint) is not str
        or len(current_memory_fingerprint) != 64
    ):
        raise FormationAdapterError(
            "invalid_writer_request", "current_memory_fingerprint must be SHA-256 text"
        )
...
zero/retry: {{"schema_version":"{PROPOSITION_WRITER_SCHEMA_VERSION}","outcome":"zero_new_propositions|retryable_defer","propositions":[],"reason_text":"brief","continuation":null}}
continuation: {{"schema_version":"{PROPOSITION_WRITER_SCHEMA_VERSION}","outcome":"incomplete_continuation","propositions":[],"reason_text":"more propositions remain","continuation":{{"remaining":true}}}}
...
context_only_evidence: {json.dumps(wire(context), ensure_ascii=False, sort_keys=True, separators=(",", ":"))}"""
    ...
    if prompt_bytes > MAX_PROMPT_BYTES:
        raise FormationAdapterError(
            "writer_prompt_overflow", "Contextual Writer prompt exceeds 64 KiB"
        )
    return {
        ...
        "context_chars": sum(
            len(item["user_utf8"]) + len(item["assistant_utf8"]) for item in context
        ),
        "field_input_count": 0,
...
def parse_proposition_writer_result(
    ...
    if (
        type(continuation_pass) is not int
        or continuation_pass < 0
        or type(evaluation_id) is not str
        or not evaluation_id
    ):
        raise FormationAdapterError(
            "invalid_writer_result", "Writer continuation identity is invalid"
        )
    keys = {"schema_version", "outcome", "propositions", "reason_text", "continuation"}
    if (
        type(value) is not dict
        or set(value) != keys
        or value.get("schema_version") != PROPOSITION_WRITER_SCHEMA_VERSION
    ...
    if value["outcome"] == "incomplete_continuation" and not value["propositions"]:
        if (
    ...
    if (
        ...
        raise FormationAdapterError(
            "invalid_writer_schema", "invalid planned Writer outcome"
        )
    ...
    if draft_ids != sorted(set(draft_ids)):
        raise FormationAdapterError(
            "invalid_writer_schema", "Writer draft IDs must be canonical and unique"
        )
    return {
        "outcome": value["outcome"],
...
def build_field_cartographer_prompt(
    ...
    if (
        ...
        or not 1 <= turn <= MAX_CARTOGRAPHER_TURNS
    ):
        raise FormationAdapterError(
            "invalid_cartographer_request",
            "Cartographer request identity or turn is invalid",
        )
    root = _workspace(memory_workspace)
    with AccessMemoryLoop(root) as loop:
        page = (
            loop.build_progressive_atlas(request_id + ":root")
            if page_value is None
            else ProgressiveAtlasPage.from_mapping(page_value)
        )
    if page.overflow:
        return {
            "status": "atlas_page_overflow",
            "retryable": True,
            "turn": turn,
    ...
    if local_detail is not None and type(local_detail) is not dict:
        raise FormationAdapterError(
            "invalid_cartographer_request", "local detail must be an object"
        )
    prompt = _cartographer_prompt(propositions, page, request_id, turn, local_detail)
    prompt_bytes = len(prompt.encode("utf-8"))
    if prompt_bytes > page.policy.max_prompt_bytes:
        raise FormationAdapterError(
            "cartographer_prompt_overflow",
            "Cartographer prompt exceeds the active byte budget",
        )
    return {
...
def advance_field_cartographer(
    ...
    if (
        ...
        raise FormationAdapterError(
            "invalid_cartographer_result", "Cartographer response fields are invalid"
        )
    ...
    if (
        type(value) is not dict
        or value.get("schema_version") != FIELD_CARTOGRAPHER_SCHEMA_VERSION
        or type(value.get("action")) is not str
    ):
        raise FormationAdapterError(
            "invalid_cartographer_schema", "invalid Cartographer envelope"
        )
    if value["action"] == "open_region":
        if (
            set(value) != {"schema_version", "action", "region_id"}
            or type(value["region_id"]) is not str
        ):
            raise FormationAdapterError(
                "invalid_cartographer_schema", "invalid open_region action"
            )
        if turn >= MAX_CARTOGRAPHER_TURNS:
            raise FormationAdapterError(
                "cartographer_turn_budget", "Cartographer turn budget is exhausted"
            )
        try:
            with AccessMemoryLoop(_workspace(memory_workspace)) as loop:
                child = loop.open_progressive_region(
                    page, value["region_id"], request_id + f":turn:{turn + 1}"
                )
        except (TypeError, ValueError) as exc:
            raise FormationAdapterError(
                "invalid_cartographer_region", str(exc)
            ) from exc
        built = build_field_cartographer_prompt(
            {"outcome": "plan", "propositions": propositions},
            memory_workspace,
            request_id,
            turn + 1,
            child.to_mapping(),
    ...
    if value["action"] == "request_local_detail":
        if (
            ...
            raise FormationAdapterError(
                "invalid_cartographer_schema", "invalid local detail action"
            )
        if turn >= MAX_CARTOGRAPHER_TURNS:
            raise FormationAdapterError(
                "cartographer_turn_budget", "Cartographer turn budget is exhausted"
        ...
        return {
            **built,
            ...
            "local_detail": detail.to_mapping(),
            "json_repair": diagnostics,
        }
    if value["action"] == "defer":
        if (
            set(value) != {"schema_version", "action", "reason_text"}
            or type(value["reason_text"]) is not str
            or not value["reason_text"]
        ):
            raise FormationAdapterError(
                "invalid_cartographer_schema", "invalid Cartographer defer"
            )
        return {
            "status": "complete",
            "outcome": "defer",
    ...
    if (
        ...
        or set(value) != {"schema_version", "action", "plans"}
        or type(value["plans"]) is not list
    ):
        raise FormationAdapterError(
            "invalid_cartographer_schema", "invalid Cartographer resolution"
        )
...
def apply_field_cartography_result(
    ...
    propositions = _writer_propositions(writer_result)
    if (
        type(cartography_result) is not dict
        or cartography_result.get("outcome") != "plan"
        or type(cartography_result.get("plans")) is not list
        or type(cartography_result.get("page")) is not dict
        or type(cartography_result.get("plan_token")) is not dict
    ):
        raise FormationAdapterError(
            "invalid_cartographer_apply", "completed Cartographer plans are required"
        )
    if type(request_id) is not str or not request_id:
        raise FormationAdapterError(
            "invalid_cartographer_apply", "request_id is required"
        )
    root = _workspace(memory_workspace)
    try:
        planned_page = ProgressiveAtlasPage.from_mapping(cartography_result["page"])
        expected_token = _cartography_plan_token(
            propositions, cartography_result["plans"], planned_page
        )
    except (TypeError, ValueError) as exc:
        raise FormationAdapterError("invalid_cartographer_apply", str(exc)) from exc
    if cartography_result["plan_token"] != expected_token:
        raise FormationAdapterError(
            "invalid_cartographer_apply", "Cartography plan token is not canonical"
        )
    with AccessMemoryLoop(root) as loop:
        current_page = loop.build_progressive_atlas(request_id + ":freshness")
        if (
            current_page.core_state_sha256 != planned_page.core_state_sha256
            or current_page.atlas_fingerprint != planned_page.atlas_fingerprint
        ):
            raise FormationAdapterError(
                "cartography_plan_stale", "Cartography field state changed before apply"
            )
        atlas = loop.build_locality_atlas(request_id + ":apply-atlas", 512)
    if atlas.overflow:
        raise FormationAdapterError(
            "atlas_overflow",
            "active apply Atlas exceeds the bounded compatibility view",
        )
    ...
    for plan in cartography_result["plans"]:
        ...
        lenses = []
        for resolution, entry_query in zip(
            plan["entry_resolutions"], proposition["entry_queries"]
        ):
            lens = {
                ...
                "basis_spans": [
                    {
                        key: span[key]
                        for key in ("capture_id", "role", "start", "end", "quote_utf8")
                    }
            ...
            candidate = next(
                (
                    item
                    for item in atlas.candidates
                    if cell
                    in [address.to_mapping() for address in item.geometry_addresses]
                ),
                None,
            )
            if candidate is None:
                raise FormationAdapterError(
                    "cartography_entry_stale",
                    "resolved Cartographer entry is absent from the apply Atlas",
                )
            path = next(
                item
                for item in atlas.paths
                if candidate.candidate_id in item.leaf_locality_candidate_ids
            )
            lenses.append(
                {
                    **lens,
        ...
        old_plans.append(
            {
                ...
                "content_utf8": proposition["content_utf8"],
                "source_capture_ids": proposition["source_capture_ids"],
                "lenses": lenses,
                "action": action,
    ...
    clean = _captures(captures)
    try:
        all_validated = validate_dream_sculptor_plans(
            request_id, old_plans, clean, atlas
        )
        ...
        if only_statement_ids is not None:
            if (
                ...
                or any(type(item) is not str for item in only_statement_ids)
            ):
                raise TypeError("only_statement_ids must be a nonempty text list")
            selected = set(only_statement_ids)
            validated = tuple(
                item for item in validated if item.statement_id in selected
            )
        ...
        for proposition, plan in zip(propositions, all_validated, strict=True):
            ...
            if plan.action == "revision_current" and plan.existing_handle is not None:
                with AccessMemoryLoop(root) as loop:
                    predecessor = loop.current_statement_for_handle(
                        plan.existing_handle
                    )
            provenance_by_statement[plan.statement.statement_id] = (
    ...
    return {
        "outcome": "plan",
        "plans": [item.to_mapping() for item in validated],
...
def _cartography_plan_token(
    ...
    writer_payload = json.dumps(
        propositions, ensure_ascii=False, sort_keys=True, separators=(",", ":")
    ).encode("utf-8")
...
def _statement_provenance(
    ...
    involved = set(proposition["source_capture_ids"]) | set(
        proposition["context_capture_ids"]
    )
    references = tuple(
        ResolvedReferenceProvenance(
            item["reference_id"],
            ...
            tuple(item["basis_evidence_ref_ids"]),
            tuple(item["timestamp_basis_capture_ids"]),
        )
    ...
    spans = tuple(
        ExactEvidenceSpan.from_mapping(item) for item in proposition["evidence_spans"]
    )
    created_at = max(
        capture_map[capture_id]["captured_epoch_ms"] for capture_id in involved
    )
    return StatementProvenance(
        ...
        predecessor,
        "exact",
        tuple(proposition["origin_kinds"]),
        tuple(proposition["derived_from_statement_ids"]),
        proposition["evaluation_id"],
        proposition["continuation_pass"],
    )
...
def _validated_context_captures(
    ...
    context = (
        []
        if context_captures is None or context_captures == []
        else _captures(context_captures)
    )
    source_ids = {item["capture_id"] for item in sources}
    if (
        len(context) > MAX_CONTEXT_CAPTURES
        or sum(len(item["user_utf8"]) + len(item["assistant_utf8"]) for item in context)
        > MAX_CONTEXT_CHARS
    ):
        raise FormationAdapterError(
            "invalid_writer_context", "Writer context exceeds its active budget"
        )
    if any(item["capture_id"] in source_ids for item in context):
        raise FormationAdapterError(
            "invalid_writer_context",
            "Writer context and absorption sources must be disjoint",
        )
    ordered = sorted(
        context, key=lambda item: (item["captured_epoch_ms"], item["capture_id"])
    )
    if ordered != context:
        raise FormationAdapterError(
            "invalid_writer_context", "Writer context must be chronological"
        )
    first_source = min(
        (item["captured_epoch_ms"], item["capture_id"]) for item in sources
    )
    if any(
        (item["captured_epoch_ms"], item["capture_id"]) >= first_source
        for item in context
    ):
        raise FormationAdapterError(
            "invalid_writer_context",
            "Writer context must precede every absorption source",
        )
    return context
        ...
        value = [
            {
                ...
                "role": role,
                "start": 0,
                "end": len(item[f"{role}_utf8"]),
                "text_utf8": item[f"{role}_utf8"],
                "window_index": 0,
                "window_count": 1,
            }
            for item in captures
            for role in ("user", "assistant")
            if item[f"{role}_utf8"]
        ]
    if type(value) is not list or not value:
        raise FormationAdapterError(
            "invalid_writer_windows", "source windows must be a non-empty list"
        )
    clean = []
    identities = set()
    for item in value:
        ...
        if (
            ...
            or set(item) != keys
            or item.get("capture_id") not in capture_map
            or item.get("role") not in {"user", "assistant"}
        ):
            raise FormationAdapterError(
                "invalid_writer_windows", "source window identity is invalid"
            )
        if any(
            type(item[name]) is not int
            for name in ("start", "end", "window_index", "window_count")
        ):
            raise FormationAdapterError(
                "invalid_writer_windows", "source window bounds are invalid"
            )
        content = capture_map[item["capture_id"]][f"{item['role']}_utf8"]
        if (
            not 0 <= item["start"] < item["end"] <= len(content)
            or content[item["start"] : item["end"]] != item["text_utf8"]
        ):
            raise FormationAdapterError(
                "invalid_writer_windows",
                "source window does not match immutable Capture",
            )
        if not 0 <= item["window_index"] < item["window_count"]:
            raise FormationAdapterError(
                "invalid_writer_windows", "source window sequence is invalid"
            )
        identity = (item["capture_id"], item["role"], item["window_index"])
        if identity in identities:
            raise FormationAdapterError(
                "invalid_writer_windows", "source window identities must be unique"
            )
        identities.add(identity)
        clean.append(item)
    return sorted(
        clean, key=lambda item: (item["capture_id"], item["role"], item["window_index"])
    )
...
def _continuation_input(value: object) -> dict[str, object]:
    ...
    if (
        type(value) is not dict
        or set(value) != keys
        or type(value["pass"]) is not int
        or value["pass"] < 0
    ):
        raise FormationAdapterError(
            "invalid_writer_continuation", "Writer continuation is invalid"
        )
    ids = value["prior_statement_ids"]
    if (
        ...
        raise FormationAdapterError(
            "invalid_writer_continuation", "prior Statement IDs must be canonical"
        )
    return value
...
def _proposition(
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormationAdapterError(
            "invalid_writer_schema", "invalid contextual Writer proposition fields"
        )
    if (
        value.get("outcome") != "statement"
        or type(value["draft_id"]) is not str
        or not value["draft_id"]
        or type(value["content_utf8"]) is not str
        or not value["content_utf8"]
    ):
        raise FormationAdapterError(
            "invalid_writer_schema",
            "Writer proposition identity and content are required",
        )
    ...
    if (
        ...
        raise FormationAdapterError(
            "invalid_writer_schema", "Writer source Capture IDs are invalid"
        )
    context_ids = value["context_capture_ids"]
    available_context_ids = {item["capture_id"] for item in context}
    if (
        type(context_ids) is not list
        or context_ids != sorted(set(context_ids))
        or any(item not in available_context_ids for item in context_ids)
    ):
        raise FormationAdapterError(
    ...
    try:
        ...
        error = (
            str(exc)
            if str(exc).startswith("evidence_quote_")
    ...
    if any(
        ...
        raise FormationAdapterError(
            "invalid_writer_schema",
            "source Evidence is outside the active source windows",
        )
    derived = value["derived_from_statement_ids"]
    available_derived = {
        statement_id
        for capture_id in source_ids
        for statement_id in sources[capture_id].get("recalled_statement_ids", [])
    }
    if (
        type(derived) is not list
        or derived != sorted(set(derived))
        or any(item not in available_derived for item in derived)
    ):
        raise FormationAdapterError(
    ...
    if type(origins) is not list or origins != sorted(expected_origins):
        raise FormationAdapterError(
    ...
    used_context_ids.update(
        ...
        for capture_id in item["timestamp_basis_capture_ids"]
        if capture_id not in source_ids
    )
    if set(context_ids) != used_context_ids:
        raise FormationAdapterError(
            "invalid_writer_schema",
            "context_capture_ids must exactly bind used context Evidence",
        )
    ...
    return {
        ...
        "derived_from_statement_ids": derived,
        "evaluation_id": evaluation_id,
        "continuation_pass": continuation_pass,
    }
...
def _legacy_proposition(
    ...
    if type(value) is not dict or set(value) != keys:
        raise FormationAdapterError(
            "invalid_writer_schema", "invalid legacy Writer proposition fields"
        )
    source_ids = value["source_capture_ids"]
    if (
        type(source_ids) is not list
    ...
    if type(value["lenses"]) is not list or not 1 <= len(value["lenses"]) <= 4:
        raise FormationAdapterError(
            "invalid_writer_schema",
            "legacy Writer proposition requires one to four Lenses",
        )
    ...
    for lens in lenses:
        for span in lens["basis_spans"]:
            identity = json.dumps(
                span, ensure_ascii=False, sort_keys=True, separators=(",", ":")
            )
    ...
    return {
        ...
        "resolved_references": [],
        "direct_queries": [
            {"query_id": lens["lens_id"], "query_utf8": lens["future_query"]}
            for lens in lenses
        ],
        "entry_queries": [
            {"query_id": lens["lens_id"], "query_utf8": lens["future_query"]}
...
def _evidence_spans(
    value: object, captures: dict[str, dict[str, object]]
) -> list[dict[str, object]]:
    if type(value) is not list or not 1 <= len(value) <= 8:
        raise FormationAdapterError(
            "invalid_writer_schema", "Writer Evidence spans are invalid"
        )
    output = []
    for span in value:
        if type(span) is not dict or set(span) != {
            ...
            raise FormationAdapterError(
                "invalid_writer_schema", "invalid Writer Evidence span"
            )
        capture = captures.get(span["capture_id"])
        if (
            capture is None
            or span["role"] not in {"user", "assistant"}
            or type(span["start"]) is not int
            or type(span["end"]) is not int
        ):
            raise FormationAdapterError(
                "invalid_writer_schema", "Writer Evidence span reference is invalid"
            )
        content = capture[f"{span['role']}_utf8"]
        if (
            not 0 <= span["start"] < span["end"] <= len(content)
            or content[span["start"] : span["end"]] != span["quote_utf8"]
        ):
            raise FormationAdapterError(
                "invalid_writer_schema",
                "Writer Evidence span does not match exact Capture text",
            )
        output.append(span)
    return output
...
def _queries(value: object, kind: str) -> list[dict[str, str]]:
    if type(value) is not list or not 1 <= len(value) <= 4:
        raise FormationAdapterError(
            "invalid_writer_schema", f"Writer {kind} queries are invalid"
        )
    if any(
        type(item) is not dict
        or set(item) != {"query_id", "query_utf8"}
        or type(item["query_id"]) is not str
        or not item["query_id"]
        or type(item["query_utf8"]) is not str
        or not item["query_utf8"]
        for item in value
    ):
        raise FormationAdapterError(
            "invalid_writer_schema", f"Writer {kind} query is invalid"
        )
    if len({item["query_id"] for item in value}) != len(value):
        raise FormationAdapterError(
            "invalid_writer_schema", f"Writer {kind} query IDs must be unique"
        )
    return value
...
def _resolved_references(
    ...
    if type(value) is not list or len(value) > 8:
        raise FormationAdapterError(
            "invalid_writer_schema", "Writer resolved references are invalid"
        )
    output = []
    for item in value:
        keys = {
        ...
        if (
            ...
            or set(item) != keys
            or item["kind"]
            not in {"temporal", "coreference", "location", "event", "other"}
            or type(item["reference_id"]) is not str
            or not item["reference_id"]
            or type(item["normalized_value"]) is not str
            or not item["normalized_value"]
        ):
            raise FormationAdapterError(
                "invalid_writer_schema", "invalid Writer resolved reference"
            )
        ...
        if (
            type(basis_ids) is not list
            or basis_ids != sorted(set(basis_ids))
            or any(ref_id not in span_ids for ref_id in basis_ids)
        ):
            raise FormationAdapterError(
                "invalid_writer_schema", "resolved reference Evidence basis is invalid"
            )
        if (
            type(timestamp_ids) is not list
            or timestamp_ids != sorted(set(timestamp_ids))
            or any(capture_id not in captures for capture_id in timestamp_ids)
        ):
            raise FormationAdapterError(
                "invalid_writer_schema", "resolved reference timestamp basis is invalid"
            )
        if not basis_ids and not timestamp_ids:
            raise FormationAdapterError(
                "invalid_writer_schema",
                "resolved reference requires explicit Evidence basis",
            )
        output.append(item)
    if len({item["reference_id"] for item in output}) != len(output):
        raise FormationAdapterError(
            "invalid_writer_schema", "resolved reference IDs must be unique"
        )
    return output
...
def _validate_absolute_dates(
    ...
    for match in re.finditer(
        r"(?<!\d)(\d{4})[-/年](\d{1,2})[-/月](\d{1,2})(?:日)?", content
    ):
        try:
            dates.add(
            ...
            raise FormationAdapterError(
                "unsupported_absolute_date", "Writer produced an invalid absolute date"
            ) from exc
    ...
    for date_text in dates:
        ...
        matching = [
            item
            for item in references
            if item["kind"] == "temporal" and item["normalized_value"] == date_text
        ]
        if not matching or not any(
            _temporal_basis_matches(date_text, item, spans, captures)
            for item in matching
        ):
            raise FormationAdapterError(
                "unsupported_absolute_date",
                f"Writer absolute date {date_text} has no Evidence basis",
            )
...
def _temporal_basis_matches(
    ...
    for capture_id in reference["timestamp_basis_capture_ids"]:
        ...
        observed = (
            datetime.fromtimestamp(capture["captured_epoch_ms"] / 1000, tz=offset)
            .date()
            .isoformat()
        )
    ...
    return any(
        span["evidence_ref_id"] in basis
        and any(variant in span["quote_utf8"] for variant in variants)
        for span in spans
    )
...
def _writer_lens(
    ...
    if (
        type(value) is not dict
        or set(value) != keys
        or type(value["lens_id"]) is not str
        or not value["lens_id"]
        or type(value["future_query"]) is not str
        or not value["future_query"]
    ):
        raise FormationAdapterError("invalid_writer_schema", "invalid Writer Lens")
    if (
        type(value["basis_spans"]) is not list
        or not 1 <= len(value["basis_spans"]) <= 8
    ):
        raise FormationAdapterError(
            "invalid_writer_schema", "Writer Lens basis spans are invalid"
        )
    for span in value["basis_spans"]:
        if type(span) is not dict or set(span) != {
            ...
            raise FormationAdapterError(
                "invalid_writer_schema", "invalid Writer Lens basis span"
            )
        capture = captures.get(span["capture_id"])
        if (
            capture is None
            ...
            raise FormationAdapterError(
                "invalid_writer_schema", "Writer Lens basis reference is invalid"
            )
        content = capture[f"{span['role']}_utf8"]
        if (
            not 0 <= span["start"] < span["end"] <= len(content)
            ...
            raise FormationAdapterError(
                "invalid_writer_schema",
                "Writer Lens basis span does not match exact Capture text",
            )
    return value
...
def _writer_propositions(value: object) -> list[dict[str, object]]:
    if (
        type(value) is not dict
        or value.get("outcome") not in {"plan", "incomplete_continuation"}
        or type(value.get("propositions")) is not list
        or not value["propositions"]
    ):
        raise FormationAdapterError(
            "invalid_writer_result", "planned Writer propositions are required"
        )
...
def _cartography_plans(
    ...
    if len(values) != len(propositions):
        raise FormationAdapterError(
            "invalid_cartographer_schema", "Cartographer must decide every proposition"
        )
    proposition_map = {item["draft_id"]: item for item in propositions}
    ...
    for value in values:
        ...
        if (
            type(value) is not dict
            or set(value) != keys
            or value.get("draft_id") not in proposition_map
            or value["draft_id"] in seen_drafts
        ):
            raise FormationAdapterError(
                "invalid_cartographer_schema", "invalid Cartographer plan identity"
            )
        seen_drafts.add(value["draft_id"])
        proposition = proposition_map[value["draft_id"]]
        if type(value["entry_resolutions"]) is not list or len(
            value["entry_resolutions"]
        ) != len(proposition["entry_queries"]):
            raise FormationAdapterError(
                "invalid_cartographer_schema",
                "Cartographer must decide every entry query",
            )
        resolutions = []
        for raw, entry_query in zip(
            value["entry_resolutions"], proposition["entry_queries"]
        ):
            if (
                type(raw) is not dict
                or set(raw) != {"entry_query_id", "region_id", "entry_id", "unresolved"}
                or raw.get("entry_query_id") != entry_query["query_id"]
                or type(raw.get("unresolved")) is not bool
            ):
                raise FormationAdapterError(
                    "invalid_cartographer_schema", "invalid entry-query resolution"
                )
            if raw["unresolved"]:
                if raw["region_id"] is not None or raw["entry_id"] is not None:
                    raise FormationAdapterError(
                        "invalid_cartographer_schema",
                        "unresolved Lens cannot claim an entry",
                    )
            ...
            if region is None:
                raise FormationAdapterError(
                    "invalid_cartographer_schema",
                    "resolved Lens selected an unavailable region",
                )
            entry = next(
                (
                    item
                    for item in region.support_entries
                    if item["entry_id"] == raw["entry_id"]
                ),
                None,
            )
            if entry is None:
                raise FormationAdapterError(
                    "invalid_cartographer_schema",
                    "resolved Lens selected an unavailable support entry",
                )
        ...
        if mode not in {
            ...
            raise FormationAdapterError(
                "invalid_cartographer_schema", "invalid Cartographer placement mode"
            )
        ...
        if len(resolved_cells) != len(set(resolved_cells)):
            raise FormationAdapterError(
                "invalid_cartographer_schema",
                "resolved Lens relation groups must be unique",
            )
        if mode == "independent_seed" and resolved_count != 0:
            raise FormationAdapterError(
                "invalid_cartographer_schema",
                "independent_seed requires every Lens unresolved",
            )
        if (
            mode in {"related_growth", "reuse", "revision_current"}
            and resolved_count == 0
        ):
            raise FormationAdapterError(
                "invalid_cartographer_schema",
                "related placement requires a resolved Lens",
            )
        if mode in {"reuse", "revision_current"} and value["existing_handle"] is None:
            raise FormationAdapterError(
                "invalid_cartographer_schema",
                "reuse and revision require an exact Handle",
            )
        if type(value["reason_text"]) is not str or not value["reason_text"]:
            raise FormationAdapterError(
                "invalid_cartographer_schema", "Cartographer reason_text is required"
            )
