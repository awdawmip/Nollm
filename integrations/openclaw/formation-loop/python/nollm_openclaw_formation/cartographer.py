...
PROPOSITION_WRITER_PROMPT_VERSION = "proposition-writer-v1-capture-only"
FIELD_CARTOGRAPHER_PROMPT_VERSION = "field-cartographer-v1-progressive-complete"
...
def build_proposition_writer_prompt(
    ...
    prompt = f"""You are Nollm's private Capture-only Proposition Writer.
Form complete Evidence-backed MemoryStatements from only the supplied immutable Captures. Never filter a complete proposition by importance, duration, or predicted usefulness. Resolve relative time from each Capture instant when possible. Generic assistant advice is not user memory unless the user supplied or adopted it.
For each Statement provide one to four future Recall Lenses. Each Lens is operation-local and cites exact Python-style Unicode character spans from supplied user or assistant text. Do not inspect or mention any field, Atlas, region, Locality, action, Handle, coordinate, Topic, entity, vector, graph, or placement.
Return exactly one raw JSON object with no markdown.
schema_version: {PROPOSITION_WRITER_SCHEMA_VERSION}
plan: {{"schema_version":"{PROPOSITION_WRITER_SCHEMA_VERSION}","outcome":"plan","propositions":[{{"draft_id":"d1","content_utf8":"complete proposition","source_capture_ids":["capture id"],"lenses":[{{"lens_id":"l1","future_query":"natural future question","basis_spans":[{{"capture_id":"capture id","role":"user|assistant","start":0,"end":1,"quote_utf8":"exact slice"}}]}}]}}],"defer_reason":null}}
...
captures: {json.dumps(capture_wire, ensure_ascii=False, sort_keys=True, separators=(',', ':'))}"""
    prompt_bytes = len(prompt.encode("utf-8"))
    if prompt_bytes > MAX_PROMPT_BYTES:
        raise FormationAdapterError("writer_prompt_overflow", "Capture-only Writer prompt exceeds 64 KiB")
    return {
...
def parse_proposition_writer_result(
    ...
    capture_map = {item["capture_id"]: item for item in clean}
    propositions = tuple(_proposition(item, capture_map) for item in value["propositions"])
    ...
    return {"outcome": "plan", "propositions": list(propositions), "defer_reason": None, "json_repair": diagnostics}
...
def apply_field_cartography_result(
    ...
    for plan in cartography_result["plans"]:
        ...
        lenses = []
        for resolution, lens in zip(plan["lens_resolutions"], proposition["lenses"]):
...
def _proposition(value: object, captures: dict[str, dict[str, object]]) -> dict[str, object]:
    keys = {"draft_id", "content_utf8", "source_capture_ids", "lenses"}
    if type(value) is not dict or set(value) != keys:
        raise FormationAdapterError("invalid_writer_schema", "invalid Writer proposition fields")
    ...
    if type(source_ids) is not list or not source_ids or source_ids != sorted(set(source_ids)) or any(item not in captures for item in source_ids):
    ...
    if type(value["lenses"]) is not list or not 1 <= len(value["lenses"]) <= 4:
        raise FormationAdapterError("invalid_writer_schema", "Writer proposition requires one to four Lenses")
    lenses = [_writer_lens(item, captures) for item in value["lenses"]]
    if len({item["lens_id"] for item in lenses}) != len(lenses):
        raise FormationAdapterError("invalid_writer_schema", "Writer Lens IDs must be unique")
    return {**value, "source_capture_ids": source_ids, "lenses": lenses}
...
Map every future Recall Lens to only geometry entries visible on the current complete Atlas page, or mark it unresolved. Representatives are navigation hints, never complete local truth. Use open_region when a region must be inspected; do not invent coordinates, regions, Topics, entities, vectors, graphs, or facts. Do not modify Statement text.
Use request_local_detail before reuse or revision_current when exact current Statements are not present. Use independent_seed only when every Lens for that complete proposition is unresolved. Use related_growth when at least one Lens resolves. reuse/revision_current require an exact visible existing_handle in local_detail_page; otherwise choose new growth, independent_seed, or defer.
Return exactly one raw JSON object with no markdown.
schema_version: {FIELD_CARTOGRAPHER_SCHEMA_VERSION}
...
resolve: {{"schema_version":"{FIELD_CARTOGRAPHER_SCHEMA_VERSION}","action":"resolve","plans":[{{"draft_id":"d1","placement_mode":"related_growth|independent_seed|reuse|revision_current|defer","lens_resolutions":[{{"lens_id":"l1","region_id":"visible region id or null","entry_id":"visible support entry id or null","unresolved":false}}],"existing_handle":null,"reason_text":"brief"}}]}}
defer: {{"schema_version":"{FIELD_CARTOGRAPHER_SCHEMA_VERSION}","action":"defer","reason_text":"brief"}}
request_id: {request_id}
turn: {turn}/{MAX_CARTOGRAPHER_TURNS}
propositions_and_lenses: {json.dumps(propositions, ensure_ascii=False, sort_keys=True, separators=(',', ':'))}
...
def _cartography_plans(values: list[object], propositions: list[dict[str, object]], page: ProgressiveAtlasPage) -> list[dict[str, object]]:
    ...
    seen_drafts = set()
    for value in values:
        keys = {"draft_id", "placement_mode", "lens_resolutions", "existing_handle", "reason_text"}
        ...
        if type(value["lens_resolutions"]) is not list or len(value["lens_resolutions"]) != len(proposition["lenses"]):
            raise FormationAdapterError("invalid_cartographer_schema", "Cartographer must decide every Lens")
        resolutions = []
        for raw, lens in zip(value["lens_resolutions"], proposition["lenses"]):
            if type(raw) is not dict or set(raw) != {"lens_id", "region_id", "entry_id", "unresolved"} or raw.get("lens_id") != lens["lens_id"] or type(raw.get("unresolved")) is not bool:
                raise FormationAdapterError("invalid_cartographer_schema", "invalid Lens resolution")
            if raw["unresolved"]:
        ...
        output.append({**value, "lens_resolutions": resolutions})
    return output
