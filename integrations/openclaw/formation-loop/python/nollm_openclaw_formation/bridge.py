...
from .cartographer import (
    ...
    read_statement_continuation_context,
)
...
def main() -> None:
    try:
        ...
        if action == "read_memory_evaluation_fingerprint":
            ...
            return
        if action == "read_statement_continuation_context":
            result = read_statement_continuation_context(
                envelope["memory_workspace"], envelope["statement_ids"]
            )
            print(
                json.dumps(
                    {"ok": True, **result}, ensure_ascii=True, separators=(",", ":")
