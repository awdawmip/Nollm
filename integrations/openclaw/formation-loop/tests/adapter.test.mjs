...
test("background absorption splits one Writer call from one bounded Cartographer session", () => {
  ...
  assert.match(body, /captureStore\.contextBefore\(records/);
  assert.match(body, /context_captures: contextCaptures/);
  assert.match(body, /captures: \[\.\.\.captures, \.\.\.contextCaptures\]/);
  assert.match(body, /writer_context_capture_count: contextCaptures\.length/);
