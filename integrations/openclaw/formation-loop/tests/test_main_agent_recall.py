...
def recall(workspace, operation_id, surface, entry, budget):
    return recall_main_agent_locality(
        str(workspace),
        operation_id,
        surface["core_state_sha256"],
        surface["atlas_fingerprint"],
        surface["page_fingerprint"],
        surface["policy"],
        entry,
        budget,
    )
...
def seed(workspace, count: int, request_id: str = "seed") -> None:
    statements = [
        {
            "statement_id": f"{request_id}-statement-{index:02d}",
            "content_utf8": f"fact {index:02d} " + "x" * 20,
            "source_handle": None,
            "context_refs": [],
        }
    ...
    built = build_batch_placement_prompt(
        statements, str(workspace), request_id, count, count
    )
    ...
    result = apply_batch_placement(
        json.dumps(
            {"schema_version": BATCH_PLACEMENT_SCHEMA_VERSION, "decisions": decisions}
        ),
        statements,
        str(workspace),
        request_id,
...
def test_main_agent_surface_hides_coordinates_and_recall_is_single_entry_bounded(
    tmp_path,
):
    seed(tmp_path, 8)
    ...
    assert all(
        "entry_cell" not in entry
        for region in surface["regions"]
        for entry in region["support_entries"]
    )
    assert all(
        "representative_statements" not in region for region in surface["regions"]
    )
    assert all(
        "statements" not in entry
        for region in surface["regions"]
        for entry in region["support_entries"]
    )
    assert surface["routing_only"] is True and surface["answer_from_surface"] is False
    assert all(
        len(region["routing_anchor_utf8"]) <= 131 for region in surface["regions"]
    )
    ...
    assert all(
        "address" not in item and "handle" not in item for item in default["items"]
    )
...
def test_nondefault_policy_is_reused_for_recall(tmp_path):
    ...
    policy = ProgressiveAtlasPolicy(
        max_regions_per_page=8, max_prompt_bytes=32768, max_depth=6
    ).to_mapping()
    surface = build_main_agent_surface(str(tmp_path), "nondefault-policy", policy)
    result = recall(
        tmp_path, "nondefault-policy", surface, surface["entries"][0], "default"
    )
