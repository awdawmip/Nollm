...
External taxonomy review recalculated the LD result as `LAB +0~5%` and
`DISTRIBUTIONS +0~5%` because the gate covered only `lab/nollm-lab/**` while
the manifest still classified broad Lab roots as ACTIVE. The later asset
taxonomy task owns any completion update; this report retains that corrected
historical result.
