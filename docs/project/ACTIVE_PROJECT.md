# Active Project Basis

- Highest principle: [NOLLM_FIRST_PRINCIPLES_AND_ANTI_DRIFT_20260711.md](../architecture/NOLLM_FIRST_PRINCIPLES_AND_ANTI_DRIFT_20260711.md)
- Active architecture: [V3.12 Unified Field Encounter](../architecture/NOLLM_ARCHITECTURE_BOOK_V3_12_UNIFIED_FIELD_ENCOUNTER_READ_WRITE_DUALITY_20260728.md)
- Content-neutral authority: [Memory Eligibility Decision](../architecture/NOLLM_CONTENT_NEUTRALITY_AND_MEMORY_ELIGIBILITY_DECISION_20260723.md)
- Core purity project book: [V3.1 Core Purity](NOLLM_PROJECT_BOOK_V3_1_CORE_PURITY_AND_EVOLVING_BASELINES_20260711.md)
- Function-priority project book: [V3.4 Core Function Priority](NOLLM_PROJECT_BOOK_V3_4_CORE_FUNCTION_PRIORITY_20260713.md)
- Active route: [V3.9 Fast Structural Geometry](NOLLM_ROUTE_BOOK_V3_9_FAST_STRUCTURAL_GEOMETRY_20260715.md)
- Current task: [Mainline and Active Asset Curation](tasks/NOLLM_C_S_T_A_H_U_O_L_D_MAINLINE_ACTIVE_ASSET_CURATION_TASK_20260730.md)
- Current status: [NOLLM_CURRENT_STATUS.md](NOLLM_CURRENT_STATUS.md)
- Canonical ledger: [NOLLM_CURRENT_STATUS_MODULE_PROGRESS_LEDGER.md](NOLLM_CURRENT_STATUS_MODULE_PROGRESS_LEDGER.md)
- Active repository asset policy: [NOLLM_ACTIVE_REPOSITORY_ASSET_POLICY.md](NOLLM_ACTIVE_REPOSITORY_ASSET_POLICY.md)
- Mainline curation report: [NOLLM_MAINLINE_ACTIVE_ASSET_CURATION_REPORT.md](NOLLM_MAINLINE_ACTIVE_ASSET_CURATION_REPORT.md)
- Current capability record: [AOLD_UNIFIED_FIELD_ENCOUNTER_REPORT.md](AOLD_UNIFIED_FIELD_ENCOUNTER_REPORT.md)
- Current ownership reallocation: [V3_12_FIELD_ENCOUNTER_REALLOCATION.md](../architecture/module-ownership/V3_12_FIELD_ENCOUNTER_REALLOCATION.md)
- Preserved Rev6.1 evidence: [AOLD_SINGLE_CONTENT_NEUTRAL_PIPELINE_REPORT.md](AOLD_SINGLE_CONTENT_NEUTRAL_PIPELINE_REPORT.md)

Current state:

```text
AOLD_UNIFIED_FIELD_ENCOUNTER_IN_PROGRESS_AT_2787525fd3325201197a72bc69bffc3427a7d8ba
```

V3.12 supersedes the split active Recall and Placement/Cartography operation
model. Rev6.1 content neutrality, exact Tool Evidence, per-Capture continuation,
single-entry geometry, and all immutable Raw Capture remain required inputs.
The unified contract is offline-validated. Provider/Host Live evidence remains
unavailable, so the task stays IN_PROGRESS rather than claiming final acceptance.
