...
This is the only active module ledger. Versioned ledgers are historical evidence and are not active status authorities.

| Module | Lifecycle | Current progress | Confidence | Preserved capability | Active gap | Task delta |
|---|---|---:|---|---|---|---:|
| CORE | CAPABILITY_VALIDATED | 95% | high for calibrated safe domain | K=96 broad policy, safe writable field, exact occupancy bands, lazy Surface | dense Recall and future capacity policy | +1% |
...
| ACCESS | CAPABILITY_VALIDATED | 95% | high for dense Surface | finite dense previews, bulk binding reads, truthful occupancy projection, safe-target preflight | dense Recall and live validation | +3% |
...
| LAB | IMPLEMENTED | 95% | high for current structural gates | broad calibration, safe-domain proof, 300-cell/1000-atom dense fixtures | dense Recall and live observation | +2% |
