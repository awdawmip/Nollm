...
Preflight is the only place where untrusted host objects are accepted. It validates CX2 plan shape before binding comparisons, then validates the nested public values used by fingerprinting and staged calls. Bad plan declarations fail as `HX1_INVALID_PLAN`; malformed host public values fail as `HX1_INVALID_BINDINGS`; malformed context fields or DG6 enablement contradictions fail as `HX1_INVALID_CONTEXT`.
...
DG6 is verification-only. The bridge projects DG6 only when a DG6 derived view is declared, an exact DG6 verification binding is supplied, and `HostExecutionContext.enable_dg6_verification` is true. If no DG6 view is declared, the host may disable DG6 and receive a completed receipt with no projection id.
