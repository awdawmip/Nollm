...
def test_hcg1_07_strict_json_rejects_unknown_missing_and_duplicates(tmp_path) -> None:
    ...
    unknown["capture"]["query_text"] = "forbidden"
...
def test_hcg1_08_ephemeral_and_current_turn_are_rejected_without_workspace_writes(tmp_path) -> None:
    ...
    payload["policy"]["lineage"] = "none"
    payload["policy"]["diagnostics"] = "off"
    result = _run_raw(tmp_path, "capture", workspace, json.dumps(payload))
    assert result["error"]["code"] == "HCG_UNSUPPORTED_CAPTURE_MODE"
    assert not workspace.exists()


def test_hcg1_c1_02_legacy_capture_request_shape_is_rejected_without_writes(tmp_path) -> None:
    workspace = tmp_path / "workspace"
    canonical = _capture_payload("hcg1_c1_legacy", "cap_hcg1_c1_legacy", "Legacy rejected.", "source:hcg1:legacy")
    legacy = {
        "request_id": canonical["request_id"],
        "capture_request": canonical["capture"],
        "capture_policy": canonical["policy"],
    }
    result = _run_raw(tmp_path, "capture", workspace, json.dumps(legacy))
    assert result["error"]["code"] == "HCG_INVALID_REQUEST"
    assert not workspace.exists()
...
def test_hcg1_09_workspace_marker_and_repo_root_boundaries_are_enforced(tmp_path) -> None:
    ...
    read_payload = {
        ...
        "version": "1",
        "request_id": "hcg1_09_read",
        "selector": {"scope": "source_window", "context_ref": "source:hcg1:repo"},
    }
...
def test_hcg1_10_error_stdout_is_single_sanitized_json_envelope(tmp_path) -> None:
    workspace = tmp_path / "workspace"
    payload = {"kind": "nollm_hcg_read_request", "version": "1", "request_id": "hcg1_10", "selector": {"scope": "source_window", "query_text": "nope"}}
...
def test_hcg1_c1_03_wrong_read_kind_and_version_are_rejected(tmp_path) -> None:
    payload = {"kind": "wrong", "version": "1", "request_id": "hcg1_bad_kind", "selector": {"scope": "source_window", "context_ref": "source:hcg1"}}
    assert _run_raw(tmp_path, "read", tmp_path / "workspace", json.dumps(payload))["error"]["code"] == "HCG_INVALID_REQUEST"
    payload["kind"] = "nollm_hcg_read_request"
    payload["version"] = "2"
    assert _run_raw(tmp_path, "read", tmp_path / "workspace", json.dumps(payload))["error"]["code"] == "HCG_INVALID_REQUEST"
