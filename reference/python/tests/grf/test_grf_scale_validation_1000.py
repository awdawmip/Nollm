...
def test_scale_validation_generator_reaches_5000_and_new_false_friends() -> None:
    ...
    assert sum(1 for item in items if item.item_id.startswith("A")) >= 1500
    assert sum(1 for item in items if item.item_id.startswith("B")) >= 2000
    assert sum(1 for item in items if item.item_id.startswith("C")) >= 1500
