...
from .real_sources import FileSourceConnector, SourceDocument
...
class IncrementalIngestion:
    ...
    def ingest_file(self, path: Path, recorded_at: str) -> IngestionResult:
        ...
        shards = []
        for window in self.connector.windows(document):
        ...
        index[document.source_id] = {"source_path": document.source_path, "source_type": document.source_type, "revision": document.revision, "window_ids": [window.window_id for window in self.connector.windows(document)], "shard_ids": shards, "retired": False}
        self._save(index)
