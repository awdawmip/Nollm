...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="nollm-grf8-perf-") as temp:
        ...
        payload={"full_ingest_ms":_ms(full),"unchanged_incremental_ms":_ms(incremental),"batch_place_ms":_ms(batch),"snapshot_restore_ms":_ms(restore),"incremental_avoids_new_shards":reload.unchanged,"batch_correct":all(item.placement_record for item in placed),"snapshot_correct":bool(restored.validate_workspace().evidence_shard_count),"source_fallback_correct":all(facade.get_source(shard) for shard in first.created_shards)}
    print(json.dumps(payload,sort_keys=True)); return 0 if all(payload[key] for key in ("incremental_avoids_new_shards","batch_correct","snapshot_correct","source_fallback_correct")) else 1
