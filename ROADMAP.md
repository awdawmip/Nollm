...
- TQ1: Bounded Complete Test Matrix / Delivery Output Convention adds a repository test delivery gate driven by a clean git commit, real pytest collection, deterministic node-id shard assignment, a non-reusable external receipt-root matrix instance, a plan-owned ignored-runtime fixture snapshot, matrix-owned detached worktree execution with failure-atomic receipts and three-marker cleanup binding, raw JUnit/frozen-input evidence verification, and a TQ1-C6 single complete-history bundle containing both the code ref and a parentless Delivery Evidence Capsule ref. It does not modify production Nollm modules or introduce runtime, OpenClaw, network, LLM/NLP, cache, database, daemon, global discovery, or automatic admission behavior.
