...
- RC1: DF1-to-DR1 RecallUniverse contract closure is implemented as a controlled repair exception. It keeps DF1 `FiniteFieldSnapshot.coarse_covers` as the DG2 local `CellRef` view while binding `RecallUniverse.covers` to same-call replayed `HexCell` values for DR1 execution. RC1 does not complete DX2; DX2 must restart from the accepted RC1 delivery head.
